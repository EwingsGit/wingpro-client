...
export default function DashboardHome() {
  ...
  const handleStatusChange = async (taskId: number, newStatus: string, taskType: 'overdue' | 'today' | 'upcoming') => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
  ...
  const formatDate = (dateString: string | null) => {
    if (!// src/components/dashboard/DashboardHome.tsx
import { useState, useEffect } from "react";
import axios from "axios";
import { toast } from "react-hot-toast";
import { Link } from "react-router-dom";
...
export default function DashboardHome() {
  ...
  const renderTaskItem = (task: Task, taskType: 'overdue' | 'today' | 'upcoming') => (
    <div
      key={task.id}
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="mt-2 flex items-center flex-wrap gap-2">
            {task.due_date && (
              <span 
                className={`text-sm ${taskType === 'overdue' ? 'text-red-500 font-medium' : 'text-gray-500'}`}
              >
  ...
  const renderTaskSection = (title: string, tasks: Task[], taskType: 'overdue' | 'today' | 'upcoming', linkPath: string, color?: string) => (
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          {renderTaskSection("Overdue Tasks", overdueTasks, 'overdue', "/dashboard/overdue", "text-red-600")}
          {renderTaskSection("Today's Tasks", todayTasks, 'today', "/dashboard/today")}
        </div>
        <div>
          {renderTaskSection("Upcoming Tasks", upcomingTasks, 'upcoming', "/dashboard/upcoming")}
        </div>
      </div>
